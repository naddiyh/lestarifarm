...
function groupByRange(
  ...
  return Array.from(map.entries()).map(([period, vals]) => ({
    period,
    ph: parseFloat((vals.reduce((a, b) => a + b, 0) / vals.length).toFixed(1)),
  }));
}

export function getPhStatus(avg: number | null): string {
  if (avg === null) return "-";
...
export function usePhChart(sensorId: number = 3) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (grouped.length > 0) {
        const total = grouped.reduce((sum, d) => sum + d.ph, 0);
        setAvgPh(parseFloat((total / grouped.length).toFixed(1)));
      } else {
        setAvgPh(null);
      }
